...
import { showAlertError, appendToTemplate, calculateBurgerTotal, calculateTrayTotal, formatCurrency } from './utils';
...
const renderTray = () => {
    ...
    const subtotal = calculateTrayTotal(tray);
...
document.querySelectorAll('#make-burguer').forEach(page => {
    ...
    if (trayEl) {
        ...
        const btnOpenTray = trayEl.querySelector('#btn-open-tray');

        btnOpenTray.addEventListener('click', e => {
            trayEl.classList.add('open');
        });

        const btnCloseTray = trayEl.querySelector('#btn-close-tray');
        
        btnCloseTray.addEventListener('click', e => {
            trayEl.classList.remove('open');
        });

        const btnPay = trayEl.querySelector('#btn-pay');
